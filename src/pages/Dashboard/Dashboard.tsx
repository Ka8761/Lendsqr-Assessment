...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (cached) {
      setUsers(JSON.parse(cached) as User[]);
      setLoading(false);
      return;
    }
  ...
  const totalPages: number = Math.ceil(filteredUsers.length / ITEMS_PER_PAGE);
  const startIndex: number = (currentPage - 1) * ITEMS_PER_PAGE;
  const endIndex: number = Math.min(
    startIndex + ITEMS_PER_PAGE,
    filteredUsers.length
  );
  const paginatedUsers: User[] = filteredUsers.slice(startIndex, endIndex);
  ...
  const getStatusStyle = (status?: UserStatus): StatusStyle => {
    ...
    if (lower === 'blacklisted')
      return { class: 'blacklisted', text: 'Blacklisted' };
